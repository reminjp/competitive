...
def interact(pstr1, pstr2, name1, name2, filename1, filename2):
    ...
    pb = subprocess.Popen(pargs2, stdin=a_to_b[0], stdout=b_to_a[1], stderr=subprocess.PIPE)
    for line in iter(pb.stderr.readline, b''):
        print(line.rstrip().decode(encoding='utf-8'))
    pa.wait()
    pb.wait()
