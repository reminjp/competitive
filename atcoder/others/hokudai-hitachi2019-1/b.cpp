...
int main() {
  ...
  for (int t = 0; t < T; t++) {
    ...
    int n_put;
    cin >> n_put;
    vector<int> put_ids(n_put);
    for (auto& e : put_ids) cin >> e;
    for (auto e : put_ids) {
      orders[e - 1].is_ordered = false;
      orders[e - 1].is_put = true;
    }
    ordered_count -= n_put;
    put_count += n_put;
    ...
    int n_archive;
    cin >> n_archive;
    vector<int> archive_ids(n_archive);
    for (auto& e : archive_ids) cin >> e;
    for (auto e : archive_ids) {
      orders[e - 1].is_put = false;
      orders[e - 1].is_achieved = true;

      put_count--;

      auto it = find(route.begin(), route.end(), orders[e - 1].dst);
