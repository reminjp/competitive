...
class Nodeless:
    ...
    def judge_play(self, responses):
        ...
        for uid, response in responses:
            if DEBUG_PRINT:
                print("JUDGE_PLAY: uid: {}, response: {}".format(uid, response, file=sys.stderr)) ##### debug
            if not response:
                continue
            ...
            if self.G.node[src]["player"] != uid:
                ...
                continue
            if num == 0:
                print("Error: [%s] invalid unit: number of moved units should not be zero" % (name,), file=sys.stderr)
                self.nodeless.append(uid)
                continue
            if self.G.node[src]["number"] < num:
                print("Error: [%s] invalid unit: node (%d) has no %d unit(s)" % (name, src, num), file=sys.stderr)
                self.nodeless.append(uid)
                continue
            if self.G.node[src]["number"] == 0:
                print("Error: [%s] invalid unit: unit to select is 1 or more" % name, file=sys.stderr)
                self.nodeless.append(uid)
                continue
            if self.G.node[dst]["player"] >= 0:
                print("Error: [%s] invalid unit: node (%d) is not free" % (name, dst), file=sys.stderr)
                self.nodeless.append(uid)
                continue
            if not self.G.has_edge(src, dst):
                print("Error: [%s] invalid unit: node (%d) and (%d) has no edge" % (name, src, dst), file=sys.stderr)
                self.nodeless.append(uid)
                continue
